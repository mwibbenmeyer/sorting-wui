...
calcountylist = base_dt$County %>% unique()
...
chunkerfast = function(skipnumber, datapath = here("Raw_data/zillow/ZTrans/Main.txt"), 
  ...
  if(filtermain){
   filtereddata=fread(datapath,
    ...
    skip = skipnumber)
    out = filtereddata[County == county]
    return(out)
} else{
    filtereddata=fread(datapath,
    sep = '|',
...
chunk3 = c(3)
chunksabb = c(8,9,10)
endpoint = numtrans - (numtrans %% 700000)
calcountylistab = calcountylist[(length(calcountylist)-4):length(calcountylist)]
for(cty in calcountylistab){
  datacheck = F
for(ch in chunksabb){
  if(ch == 10){
  ...
  transdata = pbmclapply(skipnumbers, chunkerfast, county = cty, mc.cores = (availableCores() - 1), mc.preschedule = F) %>% rbindlist()
 ...
 # Drop transactions of multiple parcels (transIDs associated with PropertySequenceNumber > 1)
  ...
  print(paste0("found observations in transdata, chunk ", ch, ". in total, there are ", nrow(transdata), ' observations for county 'cty))
  proptransdata = pbmclapply(skipnumbers, chunkerfast, filtermain = F, county = cty, datapath = here("Raw_data/zillow/ZTrans/PropertyInfo.txt"), mc.cores = (availableCores() - 1), mc.preschedule = F) %>% rbindlist()
  ...
  filename = paste0(cty ,"_chunk_", ch, ".csv")
  filepath = paste0("/Volumes/G-DRIVE mobile SSD R-Series/ZillowCountyTransactions/", filename)
...
setkeyv(dechunked_files, c("TransId", "LoadID"))
keepRows <- dechunked_files[ ,.I[.N], by = "TransId"]
trans <- dechunked_files[keepRows[[2]], ]
trans[ , LoadID:= NULL]
